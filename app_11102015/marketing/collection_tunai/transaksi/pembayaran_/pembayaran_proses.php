	...
	try
	{
		...
		if ($act == 'Tambah') # Proses Tambah
		{
			...
			while( ! $obj->EOF)
			{
				...
				$keterangan		= 'Pembayaran ANGSURAN atas pembelian '.$bangunan.' di '.$lokasi.' Blok '.$blok.' Nomor '.$nomor.' (TYPE '.$tipe.')'.' ANGSURAN : Rp. '.to_money($jumlah).',-'.' PPN : Rp. '.to_money($ppn).',-';
			...
			$query = "update PEMBAYARAN set STATUS = '1' where NOMOR_VA = '$nomor_va' AND STATUS = '0'";
