...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	try
	{
		...
		if ($act == 'Tambah') # Proses Tambah
		{
			...
			$query = "SELECT COUNT(NAMA_DESA) AS TOTAL FROM DESA WHERE NAMA_DESA = '$nama_desa'";
			ex_found($conn->Execute($query)->fields['TOTAL'], "Nama desa '$nama_desa' telah terdaftar.");
		...
		elseif ($act == 'Hapus') # Proses Hapus
		{
			...
			foreach ($cb_data as $id_del)
			{
				...
				$querySrc= "SELECT COUNT(KODE_Desa) AS TOTAL FROM stok WHERE KODE_desa ='$id_del'";
				ex_found($conn->Execute($querySrc)->fields['TOTAL'], "Kode Lokasi \'$id_del\' telah terdaftar.");

				echo "querySrc";

				$query = "DELETE FROM DESA WHERE KODE_DESA = $id_del";
