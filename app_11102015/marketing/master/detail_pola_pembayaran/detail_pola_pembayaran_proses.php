...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	try
	{
		...
		if ($act == 'Tambah') # Proses Tambah
		{
			...
			$kode_blok = strtoupper($kode_blok);
		...
		elseif ($act == 'Ubah') # Proses Ubah
		{
			...
			$harga_tanah = (isset($_REQUEST['harga_tanah'])) ? to_decimal($_REQUEST['harga_tanah']) : 0;
			$harga_bangunan = (isset($_REQUEST['harga_bangunan'])) ? to_decimal($_REQUEST['harga_bangunan']) : 0;
...
if ($act == 'Ubah')
{
	...
	$query = "SELECT * FROM DETAIL_POLA_BAYAR WHERE KODE_BLOK = '$id' AND KODE_POLA_BAYAR = '$pola_bayar'";
