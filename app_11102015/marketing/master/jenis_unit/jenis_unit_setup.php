...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'jenis_unit_load.php', data);
	return false;
}

function showPopup(act, id, nm) {
	var url = this_base + 'jenis_unit_popup.php?act=' + act + '&id=' + id + '&nm=' + nm;
	setPopup(act + ' Jenis Unit', url, 400, 160);
	return false;
}

function hapusData()
{	
	var url		= base_marketing + 'master/jenis_unit/jenis_unit_proses.php?act=Hapus',
...
<div class="title-page">DAFTAR JENIS UNIT</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1" class="wauto">
			<option value="KODE_UNIT"> KODE </option>
			<option value="JENIS_UNIT"> JENIS UNIT </option>
		</select>
