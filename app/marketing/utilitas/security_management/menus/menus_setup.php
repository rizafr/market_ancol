...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'menus_load.php', data);
	return false;
}

function showPopup(act, id) {
	var url = this_base + 'menus_popup.php?act=' + act + '&id=' + id;
	setPopup(act + ' MENU', url, 500, 300);
	return false;
}

function hapusData() {
	var url		= this_base + 'menus_proses.php?act=Hapus',
...
<div class="title-page">SECURITY MANAGEMENT (MENU)</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1">
			<option value="m.MENU_ID"> ID </option>
			<option value="m.MENU_NAME"> MENU </option>
		</select>
