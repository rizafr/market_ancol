<?php
	...
	$namafile = "[Mandiri]Daftar Penagihan "."(".$bulan.").xls";
...
<?php
	...
	while( ! $obj->EOF)
	{
		...
		$obj2 	= $conn->Execute("SELECT KODE_VA_UNIT FROM BANK WHERE NAMA_BANK = 'BANK MANDIRI'");
		$reg	= $obj2->fields['KODE_VA_UNIT'];
		$no		= $obj->fields['NO_CUSTOMER'];
		$va		= $reg.$no;
		
		?>
		<tr> 
			<td class="text-center"><?php echo $va; ?></td>
			...
			if($n_rencana == 0)
			{
				...
				";
				$obj2 				= $conn->execute($query2);				
				$rencana_sekarang	= $rencana_sekarang + $obj2->fields['NILAI'];
			...
			else
			{
				...
				{	
					...
					if($bayar_lalu == 0)
					{
						...
						";
						$obj2 				= $conn->execute($query2);				
						$rencana_kemarin2	= $rencana_kemarin2 + $obj2->fields['NILAI'];
					...
					";
					$obj2 			 	= $conn->execute($query2);				
					$rencana_kemarin 	= $rencana_kemarin + $obj2->fields['NILAI'];
			...
			//menyimpan data denda ke dalam database (untuk yang memiliki denda)	
			if($total_denda > 0)
			{			
			...
			<td class="text-center"><?php echo '01\ANGSURAN\ANGSURAN'.'&#92'.' '.floor($total_rencana); ?></td>	
			<td class="text-center"><?php echo'01\DENDA\DENDA'.'&#92'.' '.floor(($total_denda + $total_lain)); ?></td>	
