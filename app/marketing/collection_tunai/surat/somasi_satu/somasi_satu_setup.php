<div class="title-page">Daftar Tagihan Somasi Pertama </div>
...
jQuery(function($) {
	...
	$(document).on('click', '#surat', function(e) {
		...
		if (checked < 1) {
			...
			e.preventDefault();
			location.href = base_marketing + 'collection_tunai/surat/somasi_satu/surat_somasi_satu.php?' + $('#form').serialize();	
			//cetakSurat();
		}
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		e.preventDefault();
		window.open(base_marketing + 'collection_tunai/surat/somasi_satu/print_somasi_satu.php?' + $('#form').serialize());
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'collection_tunai/surat/somasi_satu/somasi_satu_load.php', data);
	return false;
}
