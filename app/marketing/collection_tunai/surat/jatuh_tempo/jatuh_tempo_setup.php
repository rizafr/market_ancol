<div class="title-page">Tagihan Jatuh Tempo </div>
...
jQuery(function($) {
	...
	$(document).on('click', '#surat', function(e) {
		...
		if (checked < 1) {
			...
			e.preventDefault();
			location.href = base_marketing + 'collection_tunai/surat/jatuh_tempo/surat_jatuh_tempo.php?' + $('#form').serialize();	
			//cetakSurat();
		}
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		e.preventDefault();
		location.href = base_marketing + 'collection_tunai/surat/jatuh_tempo/print_jatuh_tempo.php?' + $('#form').serializeArray();
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'collection_tunai/surat/jatuh_tempo/jatuh_tempo_load.php', data);
	return false;
}
