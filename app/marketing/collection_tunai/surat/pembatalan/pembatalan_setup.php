<div class="title-page">Daftar Administrasi Pembatalan</div>
...
jQuery(function($) {
	...
	$(document).on('click', '#surat', function(e) {
		...
		if (checked < 1) {
			...
			e.preventDefault();
			location.href = base_marketing + 'collection_tunai/surat/pembatalan/surat_pembatalan.php?' + $('#form').serialize();	
			//cetakSurat();
		}
		return false;
	});
	...
	$(document).on('click', '#print', function(e) {
		e.preventDefault();
		window.open(base_marketing + 'collection_tunai/surat/pembatalan/print_pembatalan.php?' + $('#form').serialize());
		return false;
	});
...
function loadData()
{
	...
	jQuery('#t-detail').load(base_marketing + 'collection_tunai/surat/pembatalan/pembatalan_load.php', data);
	return false;
}
