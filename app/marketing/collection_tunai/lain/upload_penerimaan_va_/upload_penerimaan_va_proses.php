<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		try
		{
			...
			for ($row = 2; $row <= $highestRow; ++$row) {
				...
				if ($total_data == 0) {
					...
					$nomor_va_gagal[$nomor_va]= $tanggal;
					} 
				$conn->committrans(); 
		...
		//virtual account yang gagal
		$jum = count($nomor_va_gagal);
		if($jum>0){
			foreach($nomor_va_gagal as $nomor_va_gagal => $tanggal){
				
				echo " NOMOR VA: ".$nomor_va_gagal ."   TANGGAL: ". $tanggal ." SUDAH ADA \n";
			};
		}
