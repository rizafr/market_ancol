	...
	try
	{
		...
		if ($act == 'Tambah') # Proses Tambah
		{
			...
			if(move_uploaded_file($_FILES['data_upload']['tmp_name'], $folder.$file_name)){
				...
				$query = "SELECT KODE_JENIS FROM CS_JENIS_PAIJB WHERE KODE_JENIS = '$kode_jenis'";
				ex_found($conn->Execute($query)->recordcount(), "Kode \"$kode_jenis\" telah terdaftar.");

				$query = "INSERT INTO CS_JENIS_PAIJB (KODE_JENIS, NAMA_JENIS, NAMA_FILE)
				VALUES(
					'$kode_jenis',
					'$nama_jenis',
					'$nama_file'
				...
				$msg = "Data Jenis PAIJB  \"$nama_jenis\" berhasil disimpan.";
				} else{
		...
		elseif ($act == 'Hapus') # Proses Hapus
		{
			...
			$msg = ($error) ? 'Sebagian data gagal dihapus.' : 'Data Jenis PAIJB berhasil dihapus.';
		}
...
if ($act == 'Ubah')
{
	$query = "SELECT * FROM CS_JENIS_PAIJB WHERE KODE_JENIS = '$id'";
