...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'jenis_penjualan_load.php', data);
	return false;
}

function showPopup(act, id, nm) {
	var url = this_base + 'jenis_penjualan_popup.php?act=' + act + '&id=' + id + '&nm=' + nm;
	setPopup(act + ' Jenis Penjualan', url, 370, 150);
	return false;
}

function hapusData()
{	
	var url		= base_marketing + 'master/jenis_penjualan/jenis_penjualan_proses.php?act=Hapus',
...
<div class="title-page">DAFTAR JENIS PENJUALAN</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1">
			...
			<option value="JENIS_PENJUALAN"> JENIS PENJUALAN </option>
		</select>
