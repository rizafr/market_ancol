...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'jenis_pembayaran_load.php', data);
	return false;
}

function showPopup(act, id, nm) {
	var url = this_base + 'jenis_pembayaran_popup.php?act=' + act + '&id=' + id + '&nm=' + nm;
	setPopup(act + ' Jenis Pembayaran', url, 400, 160);
	return false;
}

function hapusData()
{	
	var url		= base_marketing + 'master/jenis_pembayaran/jenis_pembayaran_proses.php?act=Hapus',
...
<div class="title-page">JENIS PEMBAYARAN</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1">
			<option value="KODE_BAYAR"> KODE </option>
			<option value="JENIS_BAYAR"> PEMBAYARAN </option>
		</select>
