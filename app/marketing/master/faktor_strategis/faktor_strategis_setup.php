...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'faktor_strategis_load.php', data);
	return false;
}

function showPopup(act, id, nm) {
	var url = this_base + 'faktor_strategis_popup.php?act=' + act + '&id=' + id + '&nm=' + nm;
	setPopup(act + ' Faktor Strategis', url, 530, 230);
	return false;
}

function hapusData()
{	
	var url		= base_marketing + 'master/faktor_strategis/faktor_strategis_proses.php?act=Hapus',
...
<div class="title-page">DAFTAR FAKTOR STRATEGIS BERDASARKAN SK</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1" class="auto">
			<option value="KODE_FAKTOR"> KODE </option>
			<option value="FAKTOR_STRATEGIS"> FAKTOR STRATEGIS </option>
		</select>
