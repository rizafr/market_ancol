...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'koordinator_load.php', data);
	return false;
}

function showPopup(act, id, nm) {
	var url = this_base + 'koordinator_popup.php?act=' + act + '&id=' + id + '&nm=' + nm;
	setPopup(act + ' Sales CO', url, 700, 450);
	return false;
}

function hapusData()
{	
	var url		= base_marketing + 'master/koordinator/koordinator_proses.php?act=Hapus',
...
<div class="title-page">KOORDINATOR</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1" class="auto">
			<option value="NOMOR_ID"> NOMOR ID </option>
