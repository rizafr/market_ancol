...
function loadData() {
	...
	jQuery('#data-load').load(this_base + 'informasi_bangunan_load.php', data);
	return false;
}

function showPopup(act, id) {
	var url = this_base + 'informasi_bangunan_popup.php?act=' + act + '&id=' + id;
	setPopup(act + ' Ubah Informasi Bangunan ', url, 1200, 600);
	return false;
}
</script>

<div class="title-page">INFORMASI BANGUNAN</div>
...
<tr>
	...
	<td>
		<select name="s_opf1" id="s_opf1" class="auto">
			...
			<option value="t.TIPE_BANGUNAN"> TIPE BANGUNAN </option>
		</select>
